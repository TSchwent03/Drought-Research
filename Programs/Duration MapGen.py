...
def create_duration_dataframe(input_dir, spi_time, drought_threshold):
    ...
    cumulative_durations = duration_df.groupby('location')['duration'].min().reset_index()
...
def duration_table_loop(SPI_time):
    ...
    try:
        ...
        while spi_key != -51:
            ...
            dfcsv.to_csv(rf'C:\Users\thoma\Documents\GitHub\Drought-Research\Tabular Data\Shortest Duration\{SPI_time}M\{spi_key/10}_{SPI_time}M_min_duration.csv', index=False)
            spi_key -= 1
    except:
        return None
